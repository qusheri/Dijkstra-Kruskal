...
public:
    ...
    HashTable(HashTable<TKey, TElement>& other);
    virtual ~HashTable();
...
private:
    ...
    public:
        ...
        Iterator& operator+(size_t steps) override {
            auto* newIt = new Iterator(*this);
            for (size_t i = 0; i < steps; i++) {
                ++(*newIt);
                if (newIt->atEnd()) break;
            }
            return *newIt;
        }
...
template<typename TKey, typename TElement>
HashTable<TKey, TElement>::HashTable(HashTable<TKey, TElement>& other)
    : table(new DynamicArraySmart<LinkedListSmart<KeyValue<TKey, TElement>>>(other.table->GetLength())), count(other.count)
{
    for (size_t i = 0; i < other.table->GetLength(); ++i) {
        LinkedListSmart<KeyValue<TKey, TElement>> newList;
        LinkedListSmart<KeyValue<TKey, TElement>>& oldList = (*other.table)[i];

        auto it = oldList.GetIterator();
        while (!it->atEnd()) {
            newList.Append(**it);
            ++(*it);
        }
